...
tuple[MaybeSequent, CloSeqs] applyWeak(CloGSequent seq, int formIdx, CloSeqs cloSeqs) {
	if (size(seq) <= 1) return <noSeq(), cloSeqs>;
	
	CloGSequent newSeq = seq - seq[formIdx];
...
tuple[MaybeSequent, CloSeqs] applyExp(CloGSequent seq, int formIdx, int nameIdx, CloSeqs cloSeqs) {
	if (term(GameLog phi, list[CloGName] a) := seq[formIdx]) {
		
		CloGSequent newSeq = seq - seq[formIdx] + term(phi, a - a[nameIdx]);
...
MaybeSequent applyIterR(CloGSequent seq, int formIdx, CloSeqs cloSeqs) {
...
MaybeSequent applyDIterR(CloGSequent seq, int formIdx, CloSeqs cloSeqs) {
...
tuple[MaybeSequent, CloSeqs] applyClo(CloGSequent seq, int formIdx, CloSeqs cloSeqs) {
	if (term(\mod(dIter(Game gamma), GameLog phi), list[CloGName] a) := seq[formIdx]) {
		for (CloGName x <- a)
			if (fpGreaterThan(cloSeqs[x].contextSeq[cloSeqs[x].fpFormulaIdx].s, \mod(dIter(gamma), phi)))
				return <noSeq(), cloSeqs>;
		CloGName newName = nameS("x", size(cloSeqs));
		cloSeqs += (newName: <seq, formIdx>);
		
		println("applied clo");
		return <sequent(seq - seq[formIdx] + term(and(phi, \mod(gamma, \mod(dIter(gamma), phi))), a + newName)), cloSeqs>;
	}
