...
// should not work since <a^x><a^x^x>p^[x_0] is in the cloSeq after the 2nd clo application, so for the 3rd one,
// to apply the clo rule to <a^x^x>p^[x_0, x_1], it must be the case that x_1 <= <a^x^x>p, thus
